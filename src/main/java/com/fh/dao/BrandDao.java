...
public interface BrandDao {
    ...
    @Update("update ds_brand set name=#{name},bandE=#{bandE},imgpath=#{imgpath}," +
            "bandDesc=#{bandDesc},ord=#{ord},isdel=#{isdel},updateDate=#{updateDate},author=#{author}" +
            "   where id=#{id}")
    void updateBrand(Brand brand);
    @Insert("insert into ds_brand (name,bandE,imgpath,bandDesc,ord,isdel,createDate,author) value(" +
